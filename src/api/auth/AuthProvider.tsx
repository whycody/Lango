...
const AuthProvider: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const storage = user?.userId
    ? useMMKV({ id: `user-${user.userId}` })
    : useMMKV();
  const [userUpdatePayload, setUserUpdatePayload] =
    useMMKVObject<UserUpdatePayload>("user-update-payload", storage);
  ...
  const sendUserUpdates = async (payload?: UserUpdatePayload) => {
    if (
      !payload ||
      (!payload.notificationsEnabled && !payload.languageLevels?.length)
    )
      return false;
    ...
    if (payload?.notificationsEnabled !== undefined) {
      const res = await updateNotificationsEnabled(
        userUpdatePayload.notificationsEnabled,
      );
      if (res) {
        if (userUpdatePayload.notificationsEnabled)
  ...
  const updateUserNotificationsEnabled = async (
    notificationsEnabled: boolean,
  ) => {
  ...
  const updateUserLanguageLevels = async (languageLevel: LanguageLevel) => {
    ...
    setUser((user) =>
      user ? { ...user, languageLevels: levelsToUpdate } : null,
    );
