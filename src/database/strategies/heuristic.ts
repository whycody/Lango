...
export const heuristicStrategy: WordSetStrategy = (
    ...
    const selectedWords = pickedStates.map(s => wordsById.get(s.wordId)).filter(Boolean) as Word[];

    const wellKnownCount = pickedStates.filter(
        s => s.studyCount > 2 && new Date(s.nextReviewDate).getTime() > now,
    ).length;

    let suggestionsToAdd = Math.min(Math.floor(size * 0.2), wellKnownCount, suggestions.length);
