...
export const mlStrategy: WordSetStrategy = (size, words, suggestions, _evaluations, wordsMLStates): WordSet => {
  const statesMap = new Map((wordsMLStates ?? []).map(s => [s.wordId, s]));
  const activeWords = words.filter(w => w.active);
  const scored = activeWords.map(w => ({ word: w, score: statesMap.get(w.id)?.gradeThreeProb ?? 0, }));
  ...
  const pickedWords = [
    ...mapWordsToSessionWords(candidates.slice(0, wordsToTake).map(w => w.word)),
