...
export function useTypedMMKV<T extends MMKVValue>(
    ...
    if (typeof defaultValue === 'string') {
        ...
        return [(value ?? defaultValue) as T, (v: T) => setValue(v as string)] as const;
    } else if (typeof defaultValue === 'number') {
        const [value, setValue] = useMMKVNumber(key, storage);
        return [(value ?? defaultValue) as T, (v: T) => setValue(v as number)] as const;
    } else {
        const [value, setValue] = useMMKVBoolean(key, storage);
        return [(value ?? defaultValue) as T, (v: T) => setValue(v as boolean)] as const;
    }
}
