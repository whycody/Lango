...
export const WordsHeuristicProvider: FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const syncWordsBatch = async (toSync: WordToSync[]): Promise<void> => {
        ...
        const updates: WordHeuristicState[] = toSync.map(({ relatedEvals, word }) =>
    ...
    const updateHeuristicState = (
        ...
        let { EF, interval, repetitionsCount, studyCount } = heuristicState;
...
export const useWordsHeuristicStates = (): WordsHeuristicContextProps => {
    ...
    if (!context)
        throw new Error('useWordsHeuristicStates must be used within a WordsHeuristicProvider');
    return context;
};
