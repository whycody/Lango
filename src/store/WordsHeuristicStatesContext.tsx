...
export const WordsHeuristicProvider: FC<{ children: ReactNode }> = ({
  children,
}) => {
  const { initialLoad } = useAppInitializer();
  const [wordsHeuristicStates, setWordsHeuristicStates] = useState<
    WordHeuristicState[]
  ...
  const { update: updateWordHeuristicState } =
    useWordsHeuristicStatesRepository();

  const { evaluations } = useEvaluations();
  const { words, langWords } = useWords();

  const langWordsIdsSet = useMemo(
    () => new Set(langWords.map((l) => l.id)),
    [langWords],
  );
  const langWordsHeuristicStates = useMemo(
    () =>
      wordsHeuristicStates?.filter((wordDetail: WordHeuristicState) =>
        langWordsIdsSet.has(wordDetail.wordId),
      ) || [],
    [wordsHeuristicStates, langWordsIdsSet],
  );

  useEffect(() => {
    if (!words || !initialized || !evaluations) return;
    const toSync = getWordsToSync(
      words,
      evaluations,
      wordsHeuristicStatesRef.current,
    );
    setLoading(true);
  ...
  const getWordsToSync = (
    ...
    currentStates: WordHeuristicState[],
  ): WordToSync[] => {
  ...
  const computeCurrentHeuristicState = (
    wordId: string,
    relatedEvals: Evaluation[],
  ): WordHeuristicState => {
  ...
  const updateHeuristicState = (
    heuristicState: WordHeuristicState,
    ...
    const nextReviewDate = new Date(
      new Date(evaluation.date).getTime() + interval * 24 * 60 * 60 * 1000,
    ).toISOString();
...
export const useWordsHeuristicStates = (): WordsHeuristicContextProps => {
  ...
  if (!context)
    throw new Error(
      "useWordsHeuristicStates must be used within a WordsHeuristicProvider",
    );
  return context;
};
