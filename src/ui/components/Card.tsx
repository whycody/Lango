...
const Card: FC<CardProps> = (props) => {
  ...
  const actionButtonIsActive = (!isSuggestion && onEditPress) || (isSuggestion && onContinuePress);
  ...
  return (
    ...
    >
      ...
      <View style={styles.textContainer}>
        <CustomText
          weight={"SemiBold"}
          style={[styles.text, text.length > 300 && styles.longText, !word && styles.exampleFlashcardText]}
          adjustsFontSizeToFit
        >
          {text}
        </CustomText>
        {onPlayAudio &&
          <Pressable style={styles.playButton} onPress={() => onPlayAudio(word, frontSide)}>
            <CustomText weight={'SemiBold'} style={styles.playText}>{t('play')}</CustomText>
            <Ionicons name={'volume-high-sharp'} color={colors.primary300} size={14}/>
          </Pressable>
        }
