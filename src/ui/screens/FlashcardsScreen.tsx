...
export const FlashcardsScreen = () => {
  ...
  const numberOfWords = wordsContext.langWords.filter(
    (word) => !word.removed,
  ).length;
  ...
  const avgGradeThreeProb = useMemo(
    () =>
      flashcards.length > 0
        ? flashcards.reduce(
            (sum, card) => sum + (card.gradeThreeProb || 0),
            0,
          ) / flashcards.length
        : 0,
    [flashcards],
  ...
  const renderFlashcardListItem = useCallback(
    ({ id, text, translation, gradeThreeProb }: WordWithDetails) => (
      <FlashcardListItem
        id={id}
        text={text}
      ...
      <View style={styles.subHeaderContainer}>
        ...
        >
          <MaterialCommunityIcons
            ...
            size={18}
          />
          <CustomText weight={"SemiBold"} style={styles.sortingLabel}>
